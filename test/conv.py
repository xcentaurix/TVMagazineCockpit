# Copyright (C) 2018-2026 by xcentaurix
# License: GNU General Public License v3.0



import json
new_channels = {}


def read_channels(resolution="hd"):
	filename = "dict_default.json"
	if filename:
		with open(filename) as data_file:
			org_channels = json.load(data_file, encoding='utf-8')
			if org_channels:
				for counter, org_channel in enumerate(org_channels):
					for resolution in ["sd", "hd", "uhd"]:
						new_channel = org_channel + "_" + resolution
						channel = {}
						channel["id"] = org_channel
						channel["tname"] = org_channels[org_channel]["tname"]
						channel["ename"] = org_channels[org_channel]["ename%s" % resolution]
						channel["service"] = org_channels[org_channel]["service%s" % resolution]
						new_channels[new_channel] = channel

				print("%s channels" % counter)
				for channel in new_channels:
					print(channel)
					print(new_channels[channel])

def write_channels():
	with open("tvspielfilm_default_channels.json", 'w') as fp:
		json.dump(new_channels, fp, indent=4)


read_channels()
write_channels()
